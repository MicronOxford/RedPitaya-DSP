...
int fpga_awg_init(void)
{
    ...
    // values chosen from generate.c in the red pitaya test libary,
    // modified to be the smallest that allows the FSM in the FPGA to work.

    // we set the ASG to create a 0 output, and then modify the buffer
    // as it reads from it. this appears to be stable.
    // as we only have 2 values in the buffer, the lag from changing
    // to the output being modified is invisable.
